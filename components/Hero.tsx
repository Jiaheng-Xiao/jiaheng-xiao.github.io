...
export function Hero() {
  ...
  return <section className="hero" id="top">
    ...
    <aside className="research-brief academic-profile" aria-label="Academic profile summary">
      <div className="brief-head"><span>Academic profile</span><span>{site.location}</span></div>
      <div className="brief-body profile-brief-body">
        <div className="profile-brief-top">
          ...
          <div><p className="brief-index">PROFILE / 01</p><h2>Jiaheng Xiao</h2><p className="profile-role">{site.role}</p></div>
        </div>
        <p className="profile-biography">I am a PhD Candidate at <a href={site.affiliation.url} target="_blank" rel="noreferrer">{site.affiliation.label} ↗</a>. I am lucky to receive mentorship from {site.mentors.map((mentor, index) => <span key={mentor.name}>{index > 0 && " & "}<a href={mentor.url} target="_blank" rel="noreferrer">{mentor.title} {mentor.name} ↗</a></span>)}. {site.bio[0]}</p>
      </div>
